...
fileManager_Main = new function() {
	...
	this.fileInit = function(thisval) {
		/*if (-1 == thisval.indexOf('.exe')) {
			Common.showBySite('温馨提示', '请选择正确的文件格式(.exe)!');
			return;
		}*/
	...
	this.progressBarPro = function() {
		var everylisten = function() {
			$.ajax({
				url : './ajaxupload/upload_listenPresent.do',
				method : 'GET',
				timeout : 120000,
					...
					if ("" != result.msg) {
						if (null != result.msg) {
							...
							if ("100%" == result.msg) {
								...
								$.ajax({
									url : './ajaxupload/upload_clearlistenPresent.do',
									method : 'GET',
									timeout : 20000,
									contentType : "application/json; charset=utf-8",
									dataType : "json",
	...
	this.showResult = function () {
		console.info('showResult');
		 ...
		 //2 终止文件上传方法
		 $.ajax({
			 url : './ajaxupload/upload_stopUpload.do',
			 method : 'GET',
			 timeout : 60000,
			 contentType : "application/json; charset=utf-8",
			 dataType : "json",
